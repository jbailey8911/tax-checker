...
INPUT_FILE = Path(os.getenv("TAX_MONITOR_INPUT", "theaters_tax_monitor_input_render_v3.json"))
...
class TexasProvider:
    ...
    def get_rates(self) -> Dict[str, float]:
        ...
        text = page_text(self.URL)
        rates: Dict[str, float] = {}

        # Examples:
        # San Antonio (Bexar Co) 2015004 .012500 .082500
        # Cibolo (Guadalupe Co) 2089071 .020000 .082500
    ...
    def _match_city_rate(self, city: str, rates: Dict[str, float]) -> Optional[float]:
        ...
        # variants like "San Antonio (Bexar Co)" or "San Antonio/Medina Co"
        prefix_matches = []
        for key, rate in rates.items():
...
class NorthCarolinaProvider:
    ...
    def get_rates(self) -> Dict[str, float]:
        ...
        text = page_text(self.URL)
        rates: Dict[str, float] = {}
        pattern = re.compile(r"^([A-Za-z]+(?: [A-Za-z]+)*)\s+(\d+(?:\.\d+)?)%[*]?$")
...
class FloridaProvider:
    ...
    def get_surtax_rates(self) -> Dict[str, float]:
        ...
        # 3 BAY 1.00 %
        # 48 SEMINOLE 1.00 %
        pattern = re.compile(r"^\d+\s+([A-Z][A-Z ]+)\s+(\d+(?:\.\d+)?)\s*%$")
